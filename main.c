...
volatile uint16 GlobalTime1ms = 0; // ms
volatile uint16 GlobalTime8ms = 0; // 8ms (& 7)
#define DESTINATION_TIME_8MS 11250 // 90000/8 1.5min
bit DestinationOk = 0;
...
#define SENSOR_VALUE_EXTREME_DECREASE_RATIO 2
// 2: 75-18=57
// 3: 75-27=48
// 4: 75-36=39
uint8 SensorValueExtreme = 0; // 用于计算减速值 | 11000000 => 5 | 00000001 => 9
uint8 SensorValueExtremeLast = 0; // 出线前的最后极值
#define MIN_EXTREME_DURATION_8MS 750 // 6s
uint16 LastMaxExtremeTime8ms = 0;
bit LastMaxExtremeTimeOk = 0;

#define SPEED_BASE 72
...
uint8 LastLedState = LED_OFF;

void Run() {
    bit dirLeft = SpeedLeft >= 0;
    bit dirRight = SpeedRight >= 0;

    PwmC0 = dirLeft ? SpeedLeft : -SpeedLeft;
    PwmC1 = dirRight ? SpeedRight : -SpeedRight;
    ...
    if (LedState != LastLedState) {
        switch (LedState) {
        case LED_LEFT:
            LedLeft = 1; LedRight = 0; break;
        case LED_RIGHT:
            LedLeft = 0; LedRight = 1; break;
        default:
            LedLeft = 0; LedRight = 0; break;
        }
        LastLedState = LedState;
    }

    // 正反转
    if (dirLeft != LastDirLeft) {
        if (dirLeft) {   // 正转
            MotorIn1 = 1;
            MotorIn2 = 0;
        } else {            // 反转
    ...
    if (dirRight != LastDirRight) {
        ...
        LastDirRight = dirRight;
    }
}

void Search() {
#define STEERING_1 8
#define STEERING_2 12
#define STEERING_3 16
#define STEERING_4 24

    // 终点判断
    if (DestinationOk && LastMaxExtremeTimeOk && SensorValueMiddle == PAT_1111) {
        // if (((SensorValueEdge >> 3) & 1) == (SensorValueEdge & 1) &&
        //     ((SensorValueEdge >> 2) & 1) == ((SensorValueEdge >> 1) & 1)) {
        //     BuzzerEnabled = 1;
        //     SpeedLeft = 0;
        //     SpeedRight = 0;
        //     Run();
...
#define LOST_LINE_DEBOUNCE_COUNT 64 // 比较时注意取绝对值
...
uint16 LastReverseTime = 0;
#define JUST_ON_LINE_LOW_SPEED_DURATION 1000
bit JustOnLine = 0; // 刚重新上线后一小段时间内降低速度
uint8 ReverseCombo = 0; // 连续尝试倒车找线的次数
// 倒车找到线后, 一段时间内的循迹取决于该模式
#define REVERSE_PATTERN_OVERWRITE_DURATION 500
bit LastReverseFound = 0;
bit UsingOverwrite = 0;
uint8 ReversePatternOverwrite = 0;

void ReverseFindLine() {
#define REVERSE_BACK_TIME_MAX 750 // 最大倒车时间
#define REVERSE_BACK_TIME_STEP 100 // combo 步长
#define REVERSE_BACK_TIME_MORE 200 // 多倒车一段时间
#define REVERSE_TURN_TIME 500 // 最大旋转时间
#define REVERSE_TURN_TIME_MIN 200 // 最小旋转时间
...
#define TURN_SPEED 50
    ...
    case REVERSE_IDLE:
        ...
        if ((uint16)(GlobalTime1ms - LastReverseTime) < REVERSE_COMBO_INTERVAL) {
            ReverseCombo++;
        } else {
            ReverseCombo = 0;
        }
    ...
    case REVERSE_BACKWARD:
        ...
        if (OnLineCountMiddle >= FOUND_LINE_DEBOUNCE_COUNT) {
            ...
            if (SensorValueExtremeLast < 0) {
                ReverseState = REVERSE_TURN_LEFT;
                ReversePatternOverwrite = 1 << 7; // 10000001 欺骗 Search 使其响应自转
                // } else if (SensorValueBalance > 0) {
            } else if (SensorValueExtremeLast > 0) {
                ReverseState = REVERSE_TURN_RIGHT;
                ReversePatternOverwrite = 1;
            } else {
        ...
        // case REVERSE_MORE_BACKWARD: // 可能不需要
            // SpeedLeft = -REVERSE_SPEED;
            // SpeedRight = -REVERSE_SPEED;
            // Run();
    ...
    case REVERSE_TURN_RIGHT:
        ...
        Run();
        if (ReverseTimer <= REVERSE_TURN_TIME_MIN) {
            break;
        }
...
void UpdateSensors() {
    ...
    if (UsingOverwrite && ReversePatternOverwrite && LastReverseFound) {
    ...
    if (SensorValueExtreme == -9 || SensorValueExtreme == 9) {
        LastMaxExtremeTime8ms = GlobalTime8ms;
    }
    ...
    if (OnLineCount >= FOUND_LINE_DEBOUNCE_COUNT) {
        // 在线时保存方向极值
        SensorValueExtremeLast = SensorValueExtreme;
    }
}
...
void main() {
    ...
    while (MotorIn1 && MotorIn2 && MotorIn3 && MotorIn4) {
        ...
        NixieBuffer[0] = SensorValueEdge / 10;
        NixieBuffer[1] = SensorValueEdge % 10;
        NixieBuffer[2] = SensorValueMiddle / 10;
        NixieBuffer[3] = SensorValueMiddle % 10;
    }
    ...
    while (1) {
        ...
        if (InAvoidance) {
            ...
            NixieBuffer[0] = ReverseState;
            NixieDisplayUint(ReversePatternOverwrite);
        } else {
            NixieBuffer[0] = 0;
            if (DestinationOk)
                NixieBuffer[0]++;
            if (LastMaxExtremeTimeOk)
                NixieBuffer[0]++;
            NixieDisplayUint(SonicDistance);
            // NixieDisplayInt(OnLineCount);
            // NixieBuffer[0] = PwmC0 / 10;
            // NixieBuffer[1] = PwmC0 % 10;
            // NixieBuffer[2] = PwmC1 / 10;
            // NixieBuffer[3] = PwmC1 % 10;
        }
        ...
        if (InAvoidance) {
            ...
            // 正常循迹
            if (JustOnLine || SensorValueMiddle == 0 && SensorValueEdge) {
                // 边亮中灭 即将离线 减速
                BaseSpeed = 50;
            } else {
                // 需要转弯时降低一定速度
                BaseSpeed = SPEED_BASE - (SensorValueExtreme << 1); // * SENSOR_VALUE_EXTREME_DECREASE_RATIO
            }
            Search();
        }
...
void Timer0_Routine() interrupt 1 {
    static bit CurMotor0, CurMotor1;
    static bit LastMotor0, LastMotor1;
    ...
    CurMotor0 = Counter < PwmC0;
    CurMotor1 = Counter < PwmC1;
    if (CurMotor0 != LastMotor0) {
        Motor0 = CurMotor0;
        LastMotor0 = CurMotor0;
    }
    if (CurMotor1 != LastMotor1) {
        Motor1 = CurMotor1;
        LastMotor1 = CurMotor1;
    }

    GlobalTime1ms++;
    if (!(GlobalTime1ms & 7)) { // % 8
        GlobalTime8ms++;
        NixieScanIsr(); // 125Hz
    }
    if (!DestinationOk)
        DestinationOk = GlobalTime8ms > DESTINATION_TIME_8MS;
    if (DestinationOk)
        LastMaxExtremeTimeOk = GlobalTime8ms - LastMaxExtremeTime8ms > MIN_EXTREME_DURATION_8MS;

    // 自然回绕比较 避免溢出出错
    if ((uint16)(GlobalTime1ms - LastReverseTime) < JUST_ON_LINE_LOW_SPEED_DURATION) {
        JustOnLine = 1;
        if ((uint16)(GlobalTime1ms - LastReverseTime) < REVERSE_PATTERN_OVERWRITE_DURATION)
            UsingOverwrite = 1;
        else
            UsingOverwrite = 0;
    } else {
    ...
    if (BuzzerEnabled && !(Counter & 1)) Buzzer = !Buzzer; // % 2
}
